...
static inline void wctdm_transmitprep(struct wctdm *wc, unsigned char ints)
{
	...
	if (ints & 0x01) 
		...
		writechunk = wc->writechunk + DAHDI_CHUNKSIZE_LOW;
...
static inline void ring_check(struct wctdm *wc, int card)
{
	...
	if (wc->modtype[card] != MOD_TYPE_FXO)
		return;
	wc->mod[card].fxo.pegtimer += DAHDI_CHUNKSIZE_LOW;
	for (x=0;x<DAHDI_CHUNKSIZE_LOW;x++) {
...
static inline void wctdm_receiveprep(struct wctdm *wc, unsigned char ints)
{
	...
	if (ints & 0x08)
		readchunk = wc->readchunk + DAHDI_CHUNKSIZE_LOW;
	else
	...
	if (!fxo->offhook) {
		if (fwringdetect) {
			...
			if ((res & 0x60) && (fxo->battery == BATTERY_PRESENT)) {
				fxo->ringdebounce += (DAHDI_CHUNKSIZE_LOW * 16);
				if (fxo->ringdebounce >= DAHDI_CHUNKSIZE_LOW * ringdebounce) {
					if (!fxo->wasringing) {
						fxo->wasringing = 1;
						dahdi_hooksig(wc->chans[card], DAHDI_RXSIG_RING);
						if (debug)
					...
					fxo->ringdebounce = DAHDI_CHUNKSIZE_LOW * ringdebounce;
				}
			} else {
				fxo->ringdebounce -= DAHDI_CHUNKSIZE_LOW * 4;
...
static inline void wctdm_proslic_check_hook(struct wctdm *wc, int card)
{
	...
	if (hook != wc->mod[card].fxs.lastrxhook) {
		...
		if (wc->mod[card].fxs.debounce > 0) {
			wc->mod[card].fxs.debounce-= 16 * DAHDI_CHUNKSIZE_LOW;
#if 0
		...
		if (wc->cardflag & (1 << x) &&
			...
			if (wc->mod[x].fxs.lasttxhook == 0x4) {
				...
				if (wc->mod[x].fxs.ohttimer) {
					wc->mod[x].fxs.ohttimer-= DAHDI_CHUNKSIZE_LOW;
					if (!wc->mod[x].fxs.ohttimer) {
...
static int wctdm_hardware_init(struct wctdm *wc)
{
	...
	outl(wc->writedma + DAHDI_CHUNKSIZE_LOW * 4 - 4, wc->ioaddr + WC_DMAWI);		/* Middle (interrupt) */
	outl(wc->writedma + DAHDI_CHUNKSIZE_LOW * 8 - 4, wc->ioaddr + WC_DMAWE);			/* End */
	
	outl(wc->readdma,                    	 wc->ioaddr + WC_DMARS);	/* Read start */
	outl(wc->readdma + DAHDI_CHUNKSIZE_LOW * 4 - 4, 	 wc->ioaddr + WC_DMARI);	/* Middle (interrupt) */
	outl(wc->readdma + DAHDI_CHUNKSIZE_LOW * 8 - 4, wc->ioaddr + WC_DMARE);	/* End */
