...
struct t1 {
	...
	unsigned char ec_chunk1[32][DAHDI_CHUNKSIZE_LOW];
	unsigned char ec_chunk2[32][DAHDI_CHUNKSIZE_LOW];
...
static int t1xxp_startup(struct dahdi_span *span)
{
	...
	for(i = 0; i < span->channels; i++)
	{
		memset(wc->ec_chunk1[i],
			DAHDI_LIN2X(0,span->chans[i]),DAHDI_CHUNKSIZE_LOW);
...
static void t1xxp_transmitprep(struct t1 *wc, int ints)
{
	...
	if (ints & 0x04 /* 0x01 */) {
		...
		txbuf = wc->writechunk + 32 * DAHDI_CHUNKSIZE_LOW;
	}
	dahdi_transmit(&wc->span);
	for (x=0;x<wc->offset;x++)
		txbuf[x] = wc->tempo[x];
	for (y=0;y<DAHDI_CHUNKSIZE_LOW;y++) {
		for (x=0;x<wc->span.channels;x++) {
			pos = y * 32 + wc->chanmap[x] + wc->offset;
			/* Put channel number as outgoing data */
			if (pos < 32 * DAHDI_CHUNKSIZE_LOW)
				txbuf[pos] = wc->chans[x]->writechunk[y];
			else
				wc->tempo[pos - 32 * DAHDI_CHUNKSIZE_LOW] = wc->chans[x]->writechunk[y];
		}
	}
}
...
static void t1xxp_receiveprep(struct t1 *wc, int ints)
{
	...
	if (ints & 0x04) {
		...
		rxbuf = wc->readchunk;
		canary = (unsigned int *)(wc->readchunk + DAHDI_CHUNKSIZE_LOW * 64 - 4);
	} else {
		rxbuf = wc->readchunk + DAHDI_CHUNKSIZE_LOW * 32;
		canary = (unsigned int *)(wc->readchunk + DAHDI_CHUNKSIZE_LOW * 32 - 4);
	}
	oldcan = *canary;
	...
	canary = (unsigned int *)(rxbuf + DAHDI_CHUNKSIZE_LOW * 32 - 4);
	*canary = (wc->canary++) | (CANARY << 16);
	for (x=0;x<wc->span.channels;x++) {
		dahdi_ec_chunk(wc->chans[x], wc->chans[x]->readchunk, wc->ec_chunk2[x]);
		memcpy(wc->ec_chunk2[x],wc->ec_chunk1[x],DAHDI_CHUNKSIZE_LOW);
		memcpy(wc->ec_chunk1[x],wc->chans[x]->writechunk,DAHDI_CHUNKSIZE_LOW);
	}
...
static int t1xxp_hardware_init(struct t1 *wc)
{
	...
	/* First frame */
	outl(wc->writedma + DAHDI_CHUNKSIZE_LOW * 32 - 4, wc->ioaddr + WC_DMAWI);		/* Middle (interrupt) */
	/* Second frame */
	outl(wc->writedma + DAHDI_CHUNKSIZE_LOW * 32 * 2 - 4, wc->ioaddr + WC_DMAWE);			/* End */
	
	outl(wc->readdma,                    	 wc->ioaddr + WC_DMARS);	/* Read start */
	/* First frame */
	outl(wc->readdma + DAHDI_CHUNKSIZE_LOW * 32 - 4, 	 wc->ioaddr + WC_DMARI);	/* Middle (interrupt) */
	/* Second frame */
	outl(wc->readdma + DAHDI_CHUNKSIZE_LOW * 32 * 2 - 4, wc->ioaddr + WC_DMARE);	/* End */
...
static int __devinit t1xxp_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	wc->readchunk = wc->writechunk + DAHDI_CHUNKSIZE_LOW * 32 * 2;
	
	/* Same thing...  */
	wc->readdma = wc->writedma + DAHDI_CHUNKSIZE_LOW * 32 * 2;
