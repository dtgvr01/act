...
(rotate_sums) "rotates" the pointers to these accumulator arrays as part
...
static int __buf_pull(struct confq *q, u_char *data, struct dahdi_chan *c, char *label)
{
	...
	if (q->outbuf < 0) {
		if (data)
		{
			if (c->flags &  DAHDI_FLAG_LINEAR)
			{
				memset(data, 0, variable_dahdi_chunk_size);
			}
			else
			{
				memset(data, DAHDI_LIN2X(0,c), variable_dahdi_chunk_size);
			}
		}
	...
	if (data)
		memcpy(data, q->buf[q->outbuf], variable_dahdi_chunk_size);
...
static void __buf_munge(struct dahdi_chan *chan, u_char *old, u_char *new)
{
	...
	for (x=0;x<variable_dahdi_chunk_size;x++) {
		...
		{
			val = x * new[x] + (variable_dahdi_chunk_size - x - 1) * old[x];
			val = val / (variable_dahdi_chunk_size - 1);
			old[x] = val;
		}
		else
		{
			val = x * DAHDI_XLAW(new[x], chan) + (variable_dahdi_chunk_size - x - 1) * DAHDI_XLAW(old[x], chan);
			val = val / (variable_dahdi_chunk_size - 1);
...
static int __buf_push(struct confq *q, u_char *data, char *label)
{
	...
	if (data)
		/* Copy in the data */
		memcpy(q->buf[q->inbuf], data, variable_dahdi_chunk_size);
...
static void reset_conf(struct dahdi_chan *chan)
{
	...
	for (x = 0; x < DAHDI_CB_SIZE; x++)
		chan->confin.buf[x] = chan->confin.buffer + variable_dahdi_chunk_size * x;

	chan->confin.inbuf = 0;
	chan->confin.outbuf = -1;

	for (x = 0; x < DAHDI_CB_SIZE; x++)
		chan->confout.buf[x] = chan->confout.buffer + variable_dahdi_chunk_size * x;
		...
		int timeout)
{
	...
	if (chan->sig == DAHDI_SIG_SF) {
		...
		chan->otimer = timeout * DAHDI_CHUNKSIZE_LOW;			/* Otimer is timer in samples */
		return;
	}
	...
	for (x = 0; x < th.count; x++) {
		...
		case MFR1_TONE:
			...
			case DAHDI_TONE_MFR1_ST3P:
				...
				t->tonesamples = 100 * DAHDI_CHUNKSIZE_LOW;
				break;
			default:
...
/*				struct hdlc_device *hdlc = chans[ch.chan]->hdlcnetdev;
	...
	case DAHDI_SET_DIALPARAMS:
	{
		...
		for (j = 0; j < ARRAY_SIZE(tone_zones); j++) {
			...
			for (i = 0; i < ARRAY_SIZE(z->dtmf); i++) {
				z->dtmf[i].tonesamples = global_dialparams.dtmf_tonelen * DAHDI_CHUNKSIZE_LOW;
			}

			/* for MFR1, we only adjust the length of the digits */
			for (i = DAHDI_TONE_MFR1_0; i <= DAHDI_TONE_MFR1_9; i++) {
				z->mfr1[i - DAHDI_TONE_MFR1_BASE].tonesamples = global_dialparams.mfv1_tonelen * DAHDI_CHUNKSIZE_LOW;
			}

			for (i = 0; i < ARRAY_SIZE(z->mfr2_fwd); i++) {
				z->mfr2_fwd[i].tonesamples = global_dialparams.mfr2_tonelen * DAHDI_CHUNKSIZE_LOW;
			}

			for (i = 0; i < ARRAY_SIZE(z->mfr2_rev); i++) {
				z->mfr2_rev[i].tonesamples = global_dialparams.mfr2_tonelen * DAHDI_CHUNKSIZE_LOW;
			}
		}
		write_unlock(&zone_lock);

		dtmf_silence.tonesamples = global_dialparams.dtmf_tonelen * DAHDI_CHUNKSIZE_LOW;
		mfr1_silence.tonesamples = global_dialparams.mfv1_tonelen * DAHDI_CHUNKSIZE_LOW;
		mfr2_silence.tonesamples = global_dialparams.mfr2_tonelen * DAHDI_CHUNKSIZE_LOW;
	...
	case DAHDI_MAINT:  /* do maintenance stuff */
	{
		...
		case DAHDI_MAINT_LOOPDOWN:
			spans[maint.spanno]->mainttimer = DAHDI_LOOPCODE_TIME * DAHDI_CHUNKSIZE_LOW;
			rv = spans[maint.spanno]->maint(spans[maint.spanno], maint.command);
			spin_unlock_irqrestore(&spans[maint.spanno]->lock, flags);
			if (rv)
				return rv;
...
static int dahdi_chanandpseudo_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long data, int unit)
{
	...
	case DAHDI_SETCONF:  /* set conf stuff */
		...
		if (chans[stack.conf.confno]) {
			if ((stack.conf.confmode & DAHDI_CONF_MODE_MASK) == DAHDI_CONF_MONITOR_RX_PREECHO ||
			    (stack.conf.confmode & DAHDI_CONF_MODE_MASK) == DAHDI_CONF_MONITOR_TX_PREECHO ||
			    (stack.conf.confmode & DAHDI_CONF_MODE_MASK) == DAHDI_CONF_MONITORBOTH_PREECHO)
				chans[stack.conf.confno]->readchunkpreec = kmalloc(sizeof(*chans[stack.conf.confno]->readchunkpreec) * variable_dahdi_chunk_size, GFP_ATOMIC);
			else {
	...
	case DAHDI_SETLINEAR:
		...
		case 0:
			...
			{
				linearmodeflag = 0 ;
				module_printk(KERN_ERR, "dahdi_base.c: Channel reset from linear mode back to x-law: 0x%x\n",chan->flags);
			}
			break;
		case 1:
			chan->flags |= DAHDI_FLAG_LINEAR;
...
static inline void __dahdi_process_getaudio_chunk(struct dahdi_chan *ss, unsigned char *txb)
{
	...
	if (ms->flags & DAHDI_FLAG_LINEAR){
		for (x=0;x<variable_dahdi_chunk_size;x+=2)
		{
			getlin[x] = (short)((txb[x+1] << 8) | txb[x]);
		}
	}
	else{
		for (x=0;x<variable_dahdi_chunk_size;x++)
	...
	if ((!ms->confmute && !ms->dialing) || (ms->flags & DAHDI_FLAG_PSEUDO)) {
		...
		case DAHDI_CONF_MONITOR:	/* Monitor a channel's rx mode */
			...
			if (ms->flags & DAHDI_FLAG_LINEAR){
				for (x=0;x<variable_dahdi_chunk_size/2;x++)
				{
					txb[x] = (unsigned char)(getlin[x] & 0xFF);
					txb[x+1] = (unsigned char)((getlin[x] >> 8) & 0xFF);
				}
			}
			else{
				for (x=0;x<variable_dahdi_chunk_size;x++)
		...
		case DAHDI_CONF_REALANDPSEUDO:
			...
			if (ms->confmode & DAHDI_CONF_PSEUDO_TALKER) {
				/* Store temp value */
				memcpy(k, getlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				/* Add conf value */
				ACSS(k, conf_sums_next[ms->_confn]);
				/* save last one */
				memcpy(ms->conflast2, ms->conflast1, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				memcpy(ms->conflast1, k, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				...
				memset(ms->conflast1, 0, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				memset(ms->conflast2, 0, DAHDI_CHUNKSIZE_LOW * sizeof(short));
			}
			memset(getlin, 0, DAHDI_CHUNKSIZE_LOW * sizeof(short));
			if (ms->flags & DAHDI_FLAG_LINEAR)
			{
				txb[0] = 0;
			}
			...
			memset(txb + 1, txb[0], variable_dahdi_chunk_size - 1);
			...
			if (ms->flags & DAHDI_FLAG_PSEUDO) /* if pseudo-channel */
			   {
				...
				if (ms->confmode & DAHDI_CONF_TALKER) {
					...
					/*  get amount actually added */
					memcpy(ms->conflast, k, DAHDI_CHUNKSIZE_LOW * sizeof(short));
					SCSS(ms->conflast, conf_sums[ms->_confn]);
					/* Really add in new value */
					ACSS(conf_sums[ms->_confn], ms->conflast);
					memcpy(ms->getlin, getlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				} else {
					memset(ms->conflast, 0, DAHDI_CHUNKSIZE_LOW * sizeof(short));
					memcpy(getlin, ms->getlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				}
				if (ms->flags & DAHDI_FLAG_LINEAR)
				{
					txb[0] = 0;
			...
			if (chans[ms->confna]->flags & DAHDI_FLAG_PSEUDO) {
				if (ms->ec_state) {
					if (ms->flags & DAHDI_FLAG_LINEAR){
						for (x=0;x<variable_dahdi_chunk_size/2;x++)
						{
							txb[x] = (unsigned char)(chans[ms->confna]->getlin[x] & 0xFF);
							txb[x+1] = (unsigned char)((chans[ms->confna]->getlin[x] >> 8) & 0xFF);
						}
					}
					else{
						for (x=0;x<variable_dahdi_chunk_size;x++)
					...
					memcpy(txb, chans[ms->confna]->getraw, variable_dahdi_chunk_size);
				}
			} else {
				if (ms->ec_state) {
					if (ms->flags & DAHDI_FLAG_LINEAR){
						for (x=0;x<variable_dahdi_chunk_size/2;x++)
						{
							txb[x] = (unsigned char)(chans[ms->confna]->putlin[x] & 0xFF);
							txb[x+1] = (unsigned char)((chans[ms->confna]->putlin[x] >> 8) & 0xFF);
						}
					}
					else{
						for (x=0;x<variable_dahdi_chunk_size;x++)
					...
					memcpy(txb, chans[ms->confna]->putraw, variable_dahdi_chunk_size);
				}
			}
			if (ms->flags & DAHDI_FLAG_LINEAR){
				for (x=0;x<variable_dahdi_chunk_size;x+=2)
	...
	memcpy(ms->getlin_lastchunk, ms->getlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
	/* save value from current */
	memcpy(ms->getlin, getlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
	/* save value from current */
	memcpy(ms->getraw, txb, DAHDI_CHUNKSIZE_LOW);
	/* if to make tx tone */
...
static inline void __dahdi_getbuf_chunk(struct dahdi_chan *ss, unsigned char *txb)
{
	...
	int bytes , left;
	int x;
	bytes = variable_dahdi_chunk_size;
...
out in the later versions, and is put back now. */
		...
		} else
		{
			if (ms->flags & DAHDI_FLAG_LINEAR)
			{
				memset(txb, 0, bytes);	/* Lastly we use silence on telephony channels */
			}
			else
			{
...
static inline void __rbs_otimer_expire(struct dahdi_chan *chan)
{
	...
	case DAHDI_TXSTATE_DEBOUNCE:
		...
		if ((chan->rxhooksig == DAHDI_RXSIG_ONHOOK) && (chan->rxflashtime > 2))
			chan->itimerset = chan->itimer = chan->rxflashtime * DAHDI_CHUNKSIZE_LOW;
		wake_up_interruptible(&chan->txstateq);
		break;
	...
	case DAHDI_TXSTATE_PULSEMAKE:
		...
		chan->otimer = chan->pulseaftertime * DAHDI_CHUNKSIZE_LOW;
		wake_up_interruptible(&chan->txstateq);
		break;
	...
	switch(chan->sig) {
...
#endif
			/* set wink timer */
			chan->itimerset = chan->itimer = chan->rxwinktime * DAHDI_CHUNKSIZE_LOW;
			break;
	   ...
	   case DAHDI_SIG_FXSKS:  /* FXS Kewlstart */
		...
		if (rxsig == DAHDI_RXSIG_ONHOOK) {
			chan->itimer = DAHDI_FXSFLASHMAXTIME * DAHDI_CHUNKSIZE_LOW;
			break;
		} else 	if (rxsig == DAHDI_RXSIG_OFFHOOK) {
			if (chan->itimer) {
				/* did the offhook occur in the window? if not, ignore both events */
				if (chan->itimer <= ((DAHDI_FXSFLASHMAXTIME - DAHDI_FXSFLASHMINTIME) * DAHDI_CHUNKSIZE_LOW))
					__qevent(chan, DAHDI_EVENT_WINKFLASH);
			}
			chan->itimer = 0;
			break;
	...
	if (ss->ec_state) {
		...
		if (ss->ec_state->status.mode & __ECHO_MODE_MUTE) {
			...
			if (ss->ec_state->ops->echocan_process) {
				...
				ss->ec_state->ops->echocan_process(ss->ec_state, rxlins, txlins, variable_dahdi_chunk_size);

				for (x = 0; x < variable_dahdi_chunk_size; x++)
				{
					if  (ss->flags &  DAHDI_FLAG_LINEAR)
					{
						rxchunk[x] = (int) rxlins[x];
					}
					else
					{
...
#define	SF_DETECT_SAMPLES (DAHDI_CHUNKSIZE_LOW * 5)
...
static inline void __dahdi_process_putaudio_chunk(struct dahdi_chan *ss, unsigned char *rxb)
{
	...
	if (ms->afterdialingtimer && (!(ms->flags & DAHDI_FLAG_PSEUDO))) {
		...
		memset(&rxb[1], rxb[0], variable_dahdi_chunk_size - 1);  /* receive as silence if dialing */
	}
	if (ms->flags & DAHDI_FLAG_LINEAR){
		for (x=0;x<variable_dahdi_chunk_size;x+=2)
		{
			putlin[x] = (short)((rxb[x+1] << 8) | rxb[x]);
		}
	}
	else{
		for (x=0;x<variable_dahdi_chunk_size;x++)
	...
	{
		r = sf_detect(&ms->rd,putlin,variable_dahdi_chunk_size,ms->rxp1,
			ms->rxp2,ms->rxp3);
		/* Convert back */
		if (ms->flags & DAHDI_FLAG_LINEAR){
			for (x=0;x<variable_dahdi_chunk_size/2;x++)
			{
				rxb[x] = (unsigned char)(putlin[x] & 0xFF);
				rxb[x+1] = (unsigned char)((putlin[x] >> 8) & 0xFF);
			}
		}
		else{
			for (x=0;x<variable_dahdi_chunk_size;x++)
	...
	if (!(ms->flags &  DAHDI_FLAG_PSEUDO)) {
		memcpy(ms->putlin, putlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
		memcpy(ms->putraw, rxb, variable_dahdi_chunk_size);
	}
	...
	if ((!ms->confmute && !ms->afterdialingtimer) ||
		...
		case DAHDI_CONF_REALANDPSEUDO:
			...
			if (ms->confmode & DAHDI_CONF_TALKER) {
				/* Store temp value */
				memcpy(k, putlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				/* Add conf value */
				ACSS(k, conf_sums_next[ms->_confn]);
				/*  get amount actually added */
				memcpy(ms->conflast, k, DAHDI_CHUNKSIZE_LOW * sizeof(short));
			...
			} else memset(ms->conflast, 0, DAHDI_CHUNKSIZE_LOW * sizeof(short));
			  /* do the pseudo-channel part processing */
			memset(putlin, 0, DAHDI_CHUNKSIZE_LOW * sizeof(short));
			...
			if (ms->flags & DAHDI_FLAG_PSEUDO) /* if a pseudo-channel */
			   {
				...
				memcpy(ss->putlin, putlin, DAHDI_CHUNKSIZE_LOW * sizeof(short));
				break;
			   }
			/* fall through */
		...
		case DAHDI_CONF_CONFANNMON:
			...
			if (ms->flags & DAHDI_FLAG_LINEAR){
				for (x=0;x<variable_dahdi_chunk_size/2;x++)
				{
					rxb[x] = (unsigned char)((int)conf_sums_prev[ms->_confn][x] & 0xFF);
					rxb[x+1] = (unsigned char)(((int)conf_sums_prev[ms->_confn][x] >> 8) & 0xFF);
				}
			}
			else{
				for (x=0;x<variable_dahdi_chunk_size;x++)
		...
		case DAHDI_CONF_DIGITALMON:
			...
			/* Add monitored channel */
			if (chans[ms->confna]->flags & DAHDI_FLAG_PSEUDO) {
				memcpy(rxb, chans[ms->confna]->getraw, variable_dahdi_chunk_size);
			} else {
				memcpy(rxb, chans[ms->confna]->putraw, variable_dahdi_chunk_size);
			}
			break;
		}
	}
...
{
	__putbuf_chunk(ss, rxb, variable_dahdi_chunk_size);
}
...
int dahdi_transmit(struct dahdi_span *span)
{
	...
	for (x=0;x<span->channels;x++) {
		...
		if (span->chans[x] == span->chans[x]->master) {
			if (span->chans[x]->otimer) {
				span->chans[x]->otimer -= DAHDI_CHUNKSIZE_LOW;
			...
			if (span->chans[x]->flags & DAHDI_FLAG_AUDIO) {
				...
				if (span->chans[x]->nextslave) {
					...
					int pos=variable_dahdi_chunk_size;
					/* Process master/slaves one way */
					for (y=0;y<variable_dahdi_chunk_size;y++) {
						/* Process slaves for this byte too */
						z = x;
						do {
							if (pos==variable_dahdi_chunk_size) {
								/* Get next chunk */
	...
	if (span->mainttimer) {
		span->mainttimer -= DAHDI_CHUNKSIZE_LOW;
	...
	for (x=0;x<span->channels;x++) {
		if (span->chans[x]->master == span->chans[x]) {
			...
			if (span->chans[x]->itimer) {
				span->chans[x]->itimer -= DAHDI_CHUNKSIZE_LOW;
			...
			if (span->chans[x]->sig & __DAHDI_SIG_FXS) {
				if (span->chans[x]->rxhooksig == DAHDI_RXSIG_RING)
					...
					span->chans[x]->ringtrailer-= DAHDI_CHUNKSIZE_LOW;
...
#ifdef BUFFER_DEBUG
			span->chans[x]->statcount -= variable_dahdi_chunk_size;
#endif
			spin_unlock_irqrestore(&span->chans[x]->lock, flags);
		}
	}
