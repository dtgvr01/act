...
struct tor2 {
	...
	unsigned char ec_chunk1[SPANS_PER_CARD][32][DAHDI_CHUNKSIZE_LOW]; /* first EC chunk buffer */
	unsigned char ec_chunk2[SPANS_PER_CARD][32][DAHDI_CHUNKSIZE_LOW]; /* second EC chunk buffer */
...
DAHDI_IRQ_HANDLER(tor2_intr)
{
	...
	for (n = 0; n < tor->spans[0].channels; n++) {
		for (i = 0; i < DAHDI_CHUNKSIZE_LOW; i++) {
			/* span 1 */
