...
static inline void pciradio_transmitprep(struct pciradio *rad, unsigned char ints)
{
	...
	if (ints & 0x01) 
		...
		writechunk = rad->writechunk + DAHDI_CHUNKSIZE_LOW;
...
static inline void pciradio_receiveprep(struct pciradio *rad, unsigned char ints)
{
	...
	if (ints & 0x08)
		readchunk = rad->readchunk + DAHDI_CHUNKSIZE_LOW;
	else
...
unsigned long endjif;
	...
	outl(rad->writedma + DAHDI_CHUNKSIZE_LOW * 4 - 4, rad->ioaddr + RAD_DMAWI);		/* Middle (interrupt) */
	outl(rad->writedma + DAHDI_CHUNKSIZE_LOW * 8 - 4, rad->ioaddr + RAD_DMAWE);			/* End */
	
	outl(rad->readdma,                    	 rad->ioaddr + RAD_DMARS);	/* Read start */
	outl(rad->readdma + DAHDI_CHUNKSIZE_LOW * 4 - 4, 	 rad->ioaddr + RAD_DMARI);	/* Middle (interrupt) */
	outl(rad->readdma + DAHDI_CHUNKSIZE_LOW * 8 - 4, rad->ioaddr + RAD_DMARE);	/* End */
