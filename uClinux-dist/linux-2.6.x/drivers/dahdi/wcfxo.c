...
struct wcfxo {
	...
	struct reg regs[DAHDI_CHUNKSIZE_LOW];
	struct reg oldregs[DAHDI_CHUNKSIZE_LOW];
	unsigned char lasttx[DAHDI_CHUNKSIZE_LOW];
...
static inline void wcfxo_transmitprep(struct wcfxo *wc, unsigned char ints)
{
	...
	memcpy(wc->lasttx, wc->chan->writechunk, DAHDI_CHUNKSIZE_LOW);

	if (ints & 0x01)  {
		/* Write is at interrupt address.  Start writing from normal offset */
		writechunk = wc->writechunk;
	} else {
		writechunk = wc->writechunk + DAHDI_CHUNKSIZE_LOW * 2;
	}
...
static inline void wcfxo_receiveprep(struct wcfxo *wc, unsigned char ints)
{
	...
	if (ints & 0x04)
		...
		readchunk = wc->readchunk + DAHDI_CHUNKSIZE_LOW * 2;

	/* Keep track of how quickly our peg alternates */
	wc->pegtimer+=DAHDI_CHUNKSIZE_LOW;
	for (x=0;x<DAHDI_CHUNKSIZE_LOW;x++) {
...
{
	int x;
	if (wc->wregcount < DAHDI_CHUNKSIZE_LOW) {
		x = wc->wregcount;
		wc->regs[x].reg = reg;
...
static int wcfxo_hardware_init(struct wcfxo *wc)
{
	...
	outl(wc->writedma + DAHDI_CHUNKSIZE_LOW * 8 - 4, wc->ioaddr + WC_DMAWI);		/* Middle (interrupt) */
	outl(wc->writedma + DAHDI_CHUNKSIZE_LOW * 16 - 4, wc->ioaddr + WC_DMAWE);			/* End */
	
	outl(wc->readdma,                    	 wc->ioaddr + WC_DMARS);	/* Read start */
	outl(wc->readdma + DAHDI_CHUNKSIZE_LOW * 8 - 4, 	 wc->ioaddr + WC_DMARI);	/* Middle (interrupt) */
	outl(wc->readdma + DAHDI_CHUNKSIZE_LOW * 16 - 4, wc->ioaddr + WC_DMARE);	/* End */
...
static void wcfxo_set_daa_mode(struct wcfxo *wc)
{
	...
	set_current_state(TASK_INTERRUPTIBLE);
	schedule_timeout(1 + (DAHDI_CHUNKSIZE_LOW * HZ) / 800);
...
//	schedule_timeout(10);
	...
	set_current_state(TASK_INTERRUPTIBLE);
	schedule_timeout(1 + HZ/1000 + (DAHDI_CHUNKSIZE_LOW * HZ) / 800);
