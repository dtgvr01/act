...
struct t4_span {
	...
	int loopdowncnt;
#ifdef SUPPORT_GEN1
	unsigned char ec_chunk1[31][DAHDI_CHUNKSIZE_LOW]; /* first EC chunk buffer */
	unsigned char ec_chunk2[31][DAHDI_CHUNKSIZE_LOW]; /* second EC chunk buffer */
#endif
	int irqmisses;
...
static void t4_receiveprep(struct t4 *wc, int irq)
{
	...
	if (irq & 1) {
		...
		readchunk = wc->readchunk + DAHDI_CHUNKSIZE_LOW * 32;
		if (wc->last0) 
			dbl = 1;
		wc->last0 = 1;
	}
	...
	if (merged) {
		for (y=0;y<ts->span.channels;y++) {
			...
			if (test_bit(y, &merged)) {
				memset(ts->span.chans[y]->readchunk, DAHDI_XLAW(0, ts->span.chans[y]), DAHDI_CHUNKSIZE_LOW);
			}
		}
	}
#endif	
...
static void t4_transmitprep(struct t4 *wc, int irq)
{
	...
	if (irq & 1) {
		...
		writechunk = wc->writechunk + DAHDI_CHUNKSIZE_LOW * 32  + 1;
	}
...
static int __devinit t4_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	/* Initialize canary */
	canary = (unsigned int *)(wc->readchunk + DAHDI_CHUNKSIZE_LOW * 64 * 4 - 4);
