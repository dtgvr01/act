...
static void ztd_sendmessage(struct dahdi_dynamic *z)
{
	...
	*((unsigned short *)buf) = htons((unsigned short)z->span.channels);
	...
	for (x=0;x<z->span.channels;x++) {
		memcpy(buf, z->chans[x]->writechunk, DAHDI_CHUNKSIZE_LOW);
		buf += DAHDI_CHUNKSIZE_LOW;
		msglen += DAHDI_CHUNKSIZE_LOW;
	}
...
void dahdi_dynamic_receive(struct dahdi_span *span, unsigned char *msg, int msglen)
{
	...
	xlen += nchans * DAHDI_CHUNKSIZE_LOW;
	...
	for (x=0;x<nchans;x++) {
		memcpy(span->chans[x]->readchunk, msg, DAHDI_CHUNKSIZE_LOW);
		msg += DAHDI_CHUNKSIZE_LOW;
	}
...
static int create_dynamic(struct dahdi_dynamic_span *zds)
{
	...
	bufsize = zds->numchans * DAHDI_CHUNKSIZE_LOW + zds->numchans / 4 + 48;
