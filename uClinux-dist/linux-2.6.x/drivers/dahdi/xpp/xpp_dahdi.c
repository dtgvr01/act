...
static const char rcsid[] = "$Id: xpp_dahdi.c 6642 2009-05-19 16:09:06Z tzafrir $";
...
static int xpd_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	if(SPAN_REGISTERED(xpd)) {
		...
		for_each_line(xpd, i) {
			...
			byte	rchunk[DAHDI_CHUNKSIZE_LOW];
			byte	wchunk[DAHDI_CHUNKSIZE_LOW];
			byte	*rp;
			byte	*wp;
			int j;
			...
			rp = chan->readchunk;
			wp = chan->writechunk;
			memcpy(rchunk, rp, DAHDI_CHUNKSIZE_LOW);
			memcpy(wchunk, wp, DAHDI_CHUNKSIZE_LOW);
			len += sprintf(page + len, "\n  port %2d>  |  ", i);
