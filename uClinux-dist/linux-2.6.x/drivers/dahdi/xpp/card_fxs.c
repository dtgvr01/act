...
static const char rcsid[] = "$Id: card_fxs.c 6659 2009-05-27 12:48:09Z tzafrir $";
...
static void detect_vmwi(xpd_t *xpd)
{
	...
	for_each_line(xpd, i) {
		...
		if(writechunk[0] != 0x7F && writechunk[0] != 0) {
			...
			for(j = 0; j < DAHDI_CHUNKSIZE_LOW; j++) {
				if(debug)
...
#endif
		if(unlikely(mem_equal(writechunk, FSK_COMMON_PATTERN, DAHDI_CHUNKSIZE_LOW))) {
			...
			oht_pcm(xpd, i, 0);
			if(unlikely(mem_equal(writechunk, FSK_ON_PATTERN, DAHDI_CHUNKSIZE_LOW))) {
				LINE_DBG(SIGNAL, xpd, i, "MSG WAITING ON\n");
				BIT_SET(xpd->msg_waiting, i);
				start_stop_vm_led(xbus, xpd, i);
			} else if(unlikely(mem_equal(writechunk, FSK_OFF_PATTERN, DAHDI_CHUNKSIZE_LOW))) {
