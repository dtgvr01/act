...
static const char rcsid[] = "$Id: card_pri.c 6659 2009-05-27 12:48:09Z tzafrir $";
...
static void pri_pcm_update(xpd_t *xpd)
{
	...
	xpd->pcm_len = RPACKET_HEADERSIZE + sizeof(xpp_line_t)  +  channels * DAHDI_CHUNKSIZE_LOW;
	...
	for_each_line(xpd, i) {
		...
		if(IS_SET(wanted_lines, i)) {
			...
			if(SPAN_REGISTERED(xpd)) {
				...
				memset((u_char *)pcm, DAHDI_XLAW(0, chan), DAHDI_CHUNKSIZE_LOW);
