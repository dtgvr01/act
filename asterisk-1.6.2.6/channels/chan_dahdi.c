...
static const struct ast_channel_tech dahdi_tech = {
	.type = "DAHDI",
	.description = tdesc,
	.capabilities = AST_FORMAT_SLINEAR | AST_FORMAT_SLINEAR16 | AST_FORMAT_ULAW | AST_FORMAT_ALAW,
...
static int send_callerid(struct dahdi_pvt *p)
{
	...
	if (p->subs[SUB_REAL].linear) {
		...
		ast_debug(1, "Channel switched to companded/linear mode to %d\n",0);
	}
...
static void dahdi_handle_dtmfup(struct ast_channel *ast, int idx, struct ast_frame **dest)
{
	...
	{
		linear_mode_flag ^= 1;
		if (linear_mode_flag == 1)
		{
			ast->nativeformats = AST_FORMAT_SLINEAR;
			ast->rawreadformat = AST_FORMAT_SLINEAR;
			ast->readformat = AST_FORMAT_SLINEAR;
			ast->rawwriteformat = AST_FORMAT_SLINEAR;
			ast->writeformat = AST_FORMAT_SLINEAR;
			p->subs[idx].linear = 1;
			dahdi_setlinear(p->subs[idx].dfd, 1);
			ast_debug(1, "Channel switched to companded/linear mode to %d\n", p->subs[idx].linear);
		}
		else
		{
			ast->nativeformats = AST_FORMAT_ULAW;
			ast->rawreadformat = AST_FORMAT_ULAW;
			ast->readformat = AST_FORMAT_ULAW;
			ast->rawwriteformat = AST_FORMAT_ULAW;
			ast->writeformat = AST_FORMAT_ULAW;
			p->subs[idx].linear = 0;
			dahdi_setlinear(p->subs[idx].dfd, 0);
			ast_debug(1, "Channel switched to companded/linear mode to %d\n", p->subs[idx].linear);
		}
	...
	if (f->subclass == '*')
		...
		linear_mode_flag ^= 1;
		ast_debug(1, "linear mode flag: to %d\n", linear_mode_flag);
			if (linear_mode_flag == 1)
			{
				ast->nativeformats = AST_FORMAT_SLINEAR16;
				ast->rawreadformat = AST_FORMAT_SLINEAR16;
				ast->readformat = AST_FORMAT_SLINEAR16;
				ast->rawwriteformat = AST_FORMAT_SLINEAR16;
				ast->writeformat = AST_FORMAT_SLINEAR16;
				p->subs[idx].linear = 2;
				dahdi_setlinear(p->subs[idx].dfd, 2);
				ast_debug(1, "Channel switched to companded/linear mode to %d\n", p->subs[idx].linear);
			}
			else
			{
...
static struct ast_frame *dahdi_read(struct ast_channel *ast)
{
	...
	if (ast->rawreadformat == AST_FORMAT_SLINEAR) {
	...
	} else if (ast->rawreadformat == AST_FORMAT_SLINEAR16) {
		if (!p->subs[idx].linear) {
			p->subs[idx].linear = 2;
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				ast_log(LOG_WARNING, "Unable to set channel %d (index %d) to linear16 mode.\n", p->channel, idx);
			else
				ast_debug(1, "Channel switched to companded/linear mode to %d\n", p->subs[idx].linear);
		}
		...
		if (p->subs[idx].linear) {
			p->subs[idx].linear = 0;
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			ast_debug(1, "Channel '%s' switched to linear mode\n" , ast->name);
			if (res)
	...
	res = read(p->subs[idx].dfd, readbuf, read_size_lcl);
	...
	p->subs[idx].f.datalen = read_size_lcl;
...
static int dahdi_write(struct ast_channel *ast, struct ast_frame *frame)
{
	...
	} else if (frame->subclass == AST_FORMAT_SLINEAR16) {
		if (!p->subs[idx].linear) {
			p->subs[idx].linear = 2;
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				ast_log(LOG_WARNING, "Unable to set linear16 mode on channel %d\n", p->channel);
			else
				ast_debug(1, "Channel switched to companded/linear mode to %d\n", p->subs[idx].linear);
		}
		res = my_dahdi_write(p, (unsigned char *)frame->data.ptr, frame->datalen, idx, 2);
	}else
	{
		/* x-law already */
		if (p->subs[idx].linear) {
		...
		for (i = iflist;; i = i->next) {
			...
			if ((i->subs[SUB_REAL].dfd > -1) && i->sig) {
				...
				if (pollres & POLLPRI) {
					...
					res = dahdi_get_event(i->subs[SUB_REAL].dfd);
					ast_debug(4, "Monitor doohicky got event %s on channel %d\n", event2str(res), i->channel);
					/* Don't hold iflock while handling init events */
					ast_mutex_unlock(&iflock);
